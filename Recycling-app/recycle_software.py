    ...
    # LLM 파트 실행 방법
    # 1. 터미널에 ollama run llava 명령어를 실행하여 Ollama LLaVA 서버를 구동합니다.
    # 2. 테스트용 이미지 파일 준비: 'images' 폴더 안에 사용할 이미지 파일들(예: test_Can.jpg, test_Plastic.jpg, test_Failure.jpg)이 있는지 확인합니다.
    # 3. 'C:\Recycling-app' 폴더에 현재 파이썬 스크립트 파일이 있다고 가정하고, 새 터미널을 열어 cd C:\Recycling-app 을 입력하여 해당 디렉토리로 이동합니다.
    # 4. Ollama 서버가 계속 실행 중인 상태로, 새 터미널에 python recycle_software.py 를 입력하여 스크립트를 실행합니다.
    # 5. 분석 결과를 확인하며 추가 질의를 하거나 다음 분석을 대기합니다.
...
def predict_image(file_path: str) -> tuple[str, float]:
    ...
    payload = {
        "model": "llava",
        "prompt": (
            "나는 대한민국 분리수거 전문 AI야! 어떤 물품이든 정확히 분류해.\n"
            "다음 사진 속 물품을 6가지 분리수거 품목 중 하나로만 분류하고, 신뢰도를 0~100% 백분율로 알려줄게.\n"
            "분류 기준은 다음과 같아. 이 기준을 최우선으로 적용하고, 물품을 정확히 식별해:\n"
            "1. *종이: 종이 상자, 신문, 책, 전단지 등. 물기에 젖지 않고, 코팅, 비닐, 테이프 등 이물질은 제거해야 해.\n"
            "2. *플라스틱: 페트병(PET), 플라스틱 용기(PP, PE, PS 등). 내용물 비우고 깨끗이 헹군 후 라벨을 제거해. **투명 비닐봉투나 과자봉지는 플라스틱이 아니라 '비닐'로 분류해.**\n"
            "3. *캔: 알루미늄 캔, 철 캔, 고철류. **금속성 재질**의 내용물을 완전히 비우고 깨끗이 헹군 후 압착해서 배출해.\n"
            "4. *유리: 유리병. 내용물 비우고 깨끗이 헹군 후 뚜껑을 제거해. **깨진 유리, 거울, 도자기, 사기그릇은 '일반쓰레기'로 분류해.**\n"
            "5. *일반쓰레기: 재활용이 불가능한 모든 품목. (예: 음식물 쓰레기, 오염 심한 비닐/종이, 깨진 유리, 도자기, 고무류, 오염된 스티로폼, 옷가지 중 재활용 불가한 것 등)\n"
            "6. *비닐: 투명/불투명 비닐봉투, 과자/라면 봉지, 랩. 내용물 비우고 깨끗이 헹궈서 배출해. **이물질 제거가 어렵거나 오염이 심하면 '일반쓰레기'로 분류해.**\n\n"
            "반드시 아래 형식만 사용해줘. 다른 설명이나 추가 문구는 절대 금지!:\n"
            "분류=[종류]\n신뢰도: [0~100]%"
        ),
        "images": [image_data],
        "stream": False
    ...
    # 분리수거 분류명 파싱
    # '캔'이 '유리'보다 먼저, 그리고 '일반쓰레기'보다 먼저 파싱되도록 순서 조정
    # 명확한 단어 우선, 그리고 '일반쓰레기'는 가능한 마지막에 확인
    for type_ in ["캔", "플라스틱", "종이", "유리", "비닐", "일반쓰레기"]:
        if type_ in text:
            label = type_
            break
    
    # 응답 텍스트에서 신뢰도 숫자 (ex. "95%")를 정규식(re)로 찾음
    match = re.search(r"(\d{1,3})\s?%", text)

    # match(신뢰도 숫자를 정상적으로 찾은 경우)가 존재한다면 
    if match:
...
def simulate_llm_response_with_vlm(query: str, image_file_path: str = None) -> str:
    ...
    if '페트병' in lower_query or 'pet' in lower_query:
        return '투명 페트병은 내용물을 비우고 깨끗하게 헹군 후 라벨을 제거하고 압착하여 투명 페트병 수거함에 넣어주세요. 다른 색깔의 페트병은 플라스틱류로 배출합니다.'
...
if __name__ == "__main__":
    ...
    print(f"질의: 이거 뭐야 (이미지: images/test_Plastic.jpg)")
    # 가정: predict_image("images/test_Plastic.jpg") -> ("플라스틱", 0.55) 반환
    guide_with_low_conf_image = simulate_llm_response_with_vlm("이거 뭐야", image_file_path="images/test_Plastic.jpg")
